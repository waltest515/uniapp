	...
	<view class="withdrawList">
		...
		<view class="count_list">
			<view class="count" v-for="(listItem,listIndex) in withdrawList" :key="listIndex">
				<!-- <view class="count_title" v-if="listItem.currency_id >= 5">{{$t('bottom.data18')}}</view>
				<view class="count_title" v-if="listItem.currency_id <= 4">{{$t('bottom.data17')}}</view> -->
				<view class="count_top">
					<view class="num">{{listItem.release_money | money}}
						<text v-if="listItem.currency_id == 5 || listItem.currency_id == 1">USDT</text>
						<text v-if="listItem.currency_id == 6 || listItem.currency_id == 2">BTCM</text>
						<text v-if="listItem.currency_id == 7 || listItem.currency_id == 3">BTC</text>
						<text v-if="listItem.currency_id == 8 || listItem.currency_id == 4">ETH</text>
					</view>
					<view class="status">
						<u--image width="148rpx" height="130rpx" src="/static/img/index/list2.png">
				...
				<view class="count_top">
					<view class="add_time" v-if="listItem.currency_id >= 5">{{$t('bottom.data18')}}</view>
					<view class="add_time" v-if="listItem.currency_id <= 4">{{$t('bottom.data17')}}</view>
	...
	export default{
		...
		methods:{
			...
			getlist(){
				let _this = this
				let urls =
					"api/Xhsk/rewardlist.html?page=" + _this.page + "&pagesize=10&address=" + _this.owerAddress
